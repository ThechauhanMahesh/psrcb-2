...
async def cancel(event):
    ...
    if not (await db.get_process(event.sender_id))["batch"]:
        return await event.reply("No batch active, all previous conversations cancelled.")
            ...
            try:
                ...
                if value > 20:
                    if not (await db.get_data(event.sender_id))["plan"] == "pro":
                        ...
                        await conv.send_message("⚠️ You can only get upto 1000 files in a single batch.")
                        return await conv.cancel_all()
            except ValueError:
